#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# File name          : TargetAllDomainObjects.py
...
def get_domain_computers(ldap_server, ldap_session, debug=False):
    ...
    for entry in ldap_session.response:
        ...
        if type(fqdn) == list:
            if debug:
                print("   [!] Computer %s has no DNS entry." % entry["attributes"]["sAMAccountName"])
        else:
            results.append(fqdn)
    return results
...
def parse_args():
    parser = argparse.ArgumentParser(add_help=True, description='Wrapper to run a command on against all users/computers/DCs of a Windows Domain.')

    parser.add_argument("targetobject", help="Target object (user, computer)")

    parser.add_argument("-c", "--command", dest="command", required=True, help="Command to launch, with {target} where the target should be placed.")
...
def worker(k, maxk, tobject, command, outfile, lock):
    try:
        if "{target}" in command and "{{target}}" not in command:
            command = command.replace("{target}", "{{target}}")
        command = jinja2.Template(command).render(target=tobject)
        print("[+] (%d/%d) Executing: %s" % (k, maxk, command))
        r = os.popen(command).read()
        lock.acquire()
        f = open(outfile, "a")
        print("[+] (%d/%d) Response of: %s" % (k, maxk, command))
        f.write("[+] (%d/%d) Response of: %s\n" % (k, maxk, command))
        print(r.strip())
        f.write(r.strip()+"\n")
        f.close()
        lock.release()
...
if __name__ == '__main__':
    ...
    results = []
    if args.targetobject == "user":
        print("[>] Extracting all users ...")
        results = get_domain_users(ldap_server, ldap_session, args.debug)
        print("[+] Found %d users." % len(results))
        print()
    elif args.targetobject == "computer":
        print("[>] Extracting all computers ...")
        results = get_domain_computers(ldap_server, ldap_session, args.debug)
        print("[+] Found %d computers." % len(results))
        print()
    ...
    try:
        with ThreadPoolExecutor(max_workers=min(args.threads, len(results))) as tp:
            kid, maxk = 0, len(results)
            for tobject in results:
                kid += 1
                tp.submit(worker, kid, maxk, tobject, args.command, args.output_file, lock)
    except KeyboardInterrupt as e:
        pass
